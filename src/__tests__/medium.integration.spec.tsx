...
describe('일정 뷰', () => {
  it('주별 뷰를 선택 후 해당 주에 일정이 없으면, 일정이 표시되지 않는다.', async () => {
    ...
    expect(within(weekView).queryByText(/\d{2}:\d{2}/)).not.toBeInTheDocument();
  });
  ...
  it('월별 뷰에 일정이 없으면, 일정이 표시되지 않아야 한다.', async () => {
    ...
    expect(within(monthView).queryByText(/\d{2}:\d{2}/)).not.toBeInTheDocument();
  });
